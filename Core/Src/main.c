...
// --- New Acquisition Variables ---
typedef enum {
    SENSOR_PHASE_ENV_START = 0,
    SENSOR_PHASE_IMU_RUN,
    SENSOR_PHASE_ENV_END
} SensorPhase;

#define SENSOR_SUPERFRAME_MS          7000UL
#define SENSOR_ENV_START_MS            300UL
#define SENSOR_IMU_END_MS             6700UL
#define SENSOR_STEP_BLE_PERIOD_MS     1000UL
...
static volatile uint8_t live_mode_active = 0U;
static volatile uint8_t live_mode_requested = 0U;
static volatile uint8_t live_mode_stop_requested = 0U;
static SensorPhase sensor_phase = SENSOR_PHASE_ENV_START;
static uint32_t sensor_phase_start_ms = 0U;
static uint32_t sensor_cycle_start_ms = 0U;
static uint32_t last_step_ble_tx_ms = 0U;
static uint8_t env_start_done = 0U;
static uint8_t env_end_done = 0U;
static uint8_t combined_metrics_pending = 0U;
static uint8_t imu_window_active = 0U;
...
/* USER CODE BEGIN 2 */
  ...
  while (1)
  {
    ...
    if (live_mode_requested != 0U)
    {
      if (live_mode_active == 0U)
      {
        LiveMode_Start(main_loop_now_ms);
      }
      live_mode_requested = 0U;
    }

    if (live_mode_stop_requested != 0U)
    {
      LiveMode_Stop();
    }
    ...
    {
      SensorSuperframe_Process(main_loop_now_ms);
    }
    ...
    UpdateStateLed(current_state);
      ...
      case STATE_IDLE:
        ...
        { /* Legacy audio-scheduler acquisition path disabled by sensor superframe.* Kept below for reference during transition. */
          HAL_StatusTypeDef start_status;
