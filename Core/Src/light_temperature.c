...
{
    // Typical relative responsivity calibration factors for AS7341 (normalized to F8)
    // F1 is the least sensitive (needs biggest boost), F8 is the most sensitive.
    float cal_factors[CHANNEL_COUNT] = {
        4.50f, // F1
        3.00f, // F2
        2.20f, // F3
        1.80f, // F4
    ...
    float cal_ch[CHANNEL_COUNT];
    float total_light = 0.0f;
    
    for (uint16_t i=0; i<CHANNEL_COUNT; i++) {
        cal_ch[i] = (float)ch[i] * cal_factors[i];
        total_light += cal_ch[i];
    }
...
uint16_t RGB_to_temperature(int * RGB_total)
{
    ...
    int rel_RGB_total[RGB_COUNT] = {0, 0, 0};
    int temp = 0;

    int max_val = RGB_total[0];
    if (RGB_total[1] > max_val) max_val = RGB_total[1];
    if (RGB_total[2] > max_val) max_val = RGB_total[2];

    if (max_val == 0) return 0;

    rel_RGB_total[0] = (int)(((uint64_t)RGB_total[0] * 255ULL) / max_val);
    rel_RGB_total[1] = (int)(((uint64_t)RGB_total[1] * 255ULL) / max_val);
    rel_RGB_total[2] = (int)(((uint64_t)RGB_total[2] * 255ULL) / max_val);

    int32_t min_dist_sq = 2000000000;
    for (uint16_t i=0; i<111; i++) {
        int32_t dr = (int32_t)kelvin_table[i][1] - rel_RGB_total[0];
        int32_t dg = (int32_t)kelvin_table[i][2] - rel_RGB_total[1];
        int32_t db = (int32_t)kelvin_table[i][3] - rel_RGB_total[2];
        int32_t dist_sq = dr*dr + dg*dg + db*db;
