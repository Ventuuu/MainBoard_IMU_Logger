...
float * get_relative_light_colors(uint16_t * ch)
{
    uint16_t total_light;
    float rel_ch[CHANNEL_COUNT];
    uint16_t rel_total_light;
    ...
    rel_total_light = rel_ch[0]+rel_ch[1]+rel_ch[2]+rel_ch[3]+rel_ch[4]+rel_ch[5]+rel_ch[6]+rel_ch[7];
    return rel_ch;
}
int * get_RGB(float * rel_ch)
{
    float RGB_ch[CHANNEL_COUNT][RGB_COUNT];
    int RGB_colors[CHANNEL_COUNT][RGB_COUNT];
    int RGB_total[RGB_COUNT] = [0, 0, 0];

    RGB_colors[0] = PURPLE;
    RGB_colors[1] = BLUE;
    RGB_colors[2] = LIGHT_BLUE;
    RGB_colors[3] = GREEN;
    RGB_colors[4] = LIGHT_GREEN;
    RGB_colors[5] = YELLOW;
    RGB_colors[6] = ORANGE;
    RGB_colors[7] = RED;

    for (i=0; i<CHANNEL_COUNT; i++) {
        for (j=0; j<RGB_COUNT; j++) {
...
uint16_t RGB_to_temperature(int * RGB_total)
{
    ...
    {
        rel_RGB_total = [255, (uint16_t)(RGB_total[1]/RGB_total[0]), (uint16_t)(RGB_total[2]/RGB_total[0])];
    }
    else if (RGB_total[2] > RGB_total[0] && RGB_total[2] > RGB_total[1])
    {
        rel_RGB_total = [(uint16_t)(RGB_total[0]/RGB_total[2]), (uint16_t)(RGB_total[1]/RGB_total[2]), 255];
    }
    if (rel_RGB_total[0] == 255)
    {
        for (i=0; i<57; i++)
        {
            if (sqrt(pow(kelvin_table[j][2] - rel_RGB_total[1], 2))<tol_G && sqrt(pow(kelvin_table[j][3] - rel_RGB_total[2], 2))<tol_RB)
            {
                temp = kelvin_table[j][0];
            }
        }
    }
    else if (rel_RGB_total[2] == 255)
    {
        for (i=57; i<111; i++)
        {
            if (sqrt(pow(kelvin_table[j][2] - rel_RGB_total[1], 2))<tol_G && sqrt(pow(kelvin_table[j][1] - rel_RGB_total[0], 2))<tol_RB)
            {
                temp = kelvin_table[j][0];
            }
        }
...
int * remove_outsiders(int * temps)
{
